...
class FakeAsrDriverTest {
    ...
    @Test
    fun `scripted audio error becomes deterministic session failure`() = runTest {
        val driver = FakeAsrDriver(
            mapOf(
                FakeDriverTrigger.CONNECT to listOf(FakeDriverStep.Emit(DriverSignal.Ready)),

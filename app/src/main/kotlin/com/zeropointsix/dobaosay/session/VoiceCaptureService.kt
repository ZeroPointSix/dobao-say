...
class VoiceCaptureService : Service() {
        ...
        try {
            ...
            if (session.snapshot.value.state !is AsrSessionState.Closed) {
                if (stopRequested.get() || !VoiceSessionBus.holdPressed.get()) {
                    ...
                    // Still "holding" but capture ended without stop — avoid close() wiping a
                    // Failed outcome that captureAndPush already published.
                    when (val state = session.snapshot.value.state) {
                        is AsrSessionState.Closed -> Unit
                        else -> session.close()
                    }
                }
                val closed =
                ...
                if (closed == null && session.snapshot.value.state !is AsrSessionState.Closed) {
                    publish(
                        VoicePhase.Failed,
                        "错误",
                        "等待最终结果超时，请重试。",
                    ...
                    updateNotification("错误", "等待最终结果超时", showStop = false)
                    runCatching { session.cancel(CancelReason.USER) }
                }
            }
        } finally {
            prebufferJob.cancel()
    ...
    private fun handleAsrEvent(
        ...
        when (event) {
            ...
            is AsrEvent.Partial -> {
                ...
                publishTranscriptUpdate(text, sessionId)
            }
            ...
            is AsrEvent.SpeechEnded -> {
                if (VoiceSessionBus.latest.phase == VoicePhase.Stopping) {
                    // Stay on 识别优化中 while finalizing.
                } else {
    ...
    private fun publishTranscriptUpdate(
        text: String,
        sessionId: String,
    ) {
        if (VoiceSessionBus.latest.phase == VoicePhase.Stopping) {
            publish(VoicePhase.Stopping, "识别优化中", text, sessionId = sessionId)
            updateNotification("识别优化中", text.take(40), showStop = false)
        } else {

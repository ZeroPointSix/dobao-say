...
class CredentialVaultTest {
    ...
    @Test
    fun `public secret path accepts bytes and redacts every value string`() =
        runTest {
            ...
            assertFalse(
                CredentialVault::class.java.methods.any { method ->
    ...
    @Test
    fun `missing expired at boundary corrupt and unavailable are distinct`() =
        runTest {
    ...
    @Test
    fun `seal and store failure preserve previous readable revision`() =
        runTest {
            val store = InMemorySealedCredentialStore()
            val protector = TestProtector()
            val vault = vault(store, protector)
    ...
    @Test
    fun `revision conflict is observable and does not overwrite current value`() =
        runTest {
            val vault = vault()
            val first = assertIs<CredentialWriteResult.Written>(write(vault))
            val second =
                assertIs<CredentialWriteResult.Written>(
    ...
    @Test
    fun `concurrent if absent has exactly one winner`() =
        runTest {
            val vault = vault()
    ...
    @Test
    fun `delete clear and lease close are idempotent`() =
        runTest {
            val vault = vault()
            write(vault)
    ...
    @Test
    fun `underlying exception marker never reaches results diagnostics or strings`() =
        runTest {
    ...
    @Test
    fun `cancelling seal leaves no value and job completes`() =
        runTest {
    ...
    @Test
    fun `cancelling before store commit preserves old version`() =
        runTest {
            val store = InMemorySealedCredentialStore()
    ...
    @Test
    fun `public scoped byte access wipes temporary arrays`() =
        runTest {
    ...
    @Test
    fun `test protector wipes transformed seal and open arrays`() =
        runTest {
            val protector = TestProtector()
            val vault = vault(protector = protector)
    ...
    @Test
    fun `throwing diagnostics never changes committed business results`() =
        runTest {
